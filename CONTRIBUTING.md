...
**PR titles must follow [Conventional Commits](https://www.conventionalcommits.org/)** (`feat:`, `fix:`, `docs:`, `chore:`, etc., with `feat!:` or a `BREAKING CHANGE:` footer for a breaking change) — this repo uses squash-merge, so your PR title becomes the commit message on `main`, and [release-please](https://github.com/googleapis/release-please) reads that history to decide the next version number and generate `CHANGELOG.md` automatically. A CI check enforces this on the PR title itself, so you'll get fast feedback if it doesn't match.
